		...
		return this.each(function(){
			...
			function initBox(){
				for(var i=0;i<count;i++){
					...
					var h=zone.height();
					var p=(i==0)?"b":"s";
					var img=$("<img src='images/photo_"+p+"_"+(i+1)+".jpg' width='"+w+"' height='"+h+"' />");
					img.animate({width:w,height:h},{duration:0});
					box.append(img);
					//box.css({"background":"url(images/photo_"+p+"_"+(i+1)+".jpg) no-repeat"});
			...
			function moveBox(dir){
				...
				for(var i=0;i<count;i++){
					...
					var p=(i==0)?"b":"s";
					box.find("img").attr("src","images/photo_"+p+"_"+((i+index)%count+1)+".jpg")
					.animate({width:w,height:h},{duration:speed});
					//box.css({"background":"url(images/photo_"+p+"_"+((i+index)%count+1)+".jpg) no-repeat"});
